"""
simulation_engine.py – v3.1 (Cleaned & Slippage-Adjusted)
...
PORTFOLIO: dict[str, dict] = {
    "INOD": {"layer": "L1 Data Engineering", "desc": "LLM Training Data Prep (Inoddata)"},
...
MA_SHORT = 60
LOOKBACK_DAYS = 63
WEIGHTS_SCHEDULE = [0.50, 0.30, 0.20]
TRAIL_STOP_PCT = 0.20
...
GIT_COMMIT_MSG = "fix: cleaned dict keys, added realistic slippage, removed emotional triggers"
...
# ─────────────────────────────────────────────────────────────────────────────
def run_dynamic_backtest(port_prices: pd.DataFrame, bm_prices: pd.DataFrame, initial_capital: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    bm_shares = {bm: initial_capital / float(bm_prices[bm].iloc[0]) for bm in bm_prices.columns}
    ...
    for date, row in port_prices.iterrows():
        ...
        if date in rebal_dates:
            ...
            nav = cash_with_interest + sum(shares[t] * today[t] for t in tickers if shares[t] > 0)
            ...
            for t, w in alloc_w.items():
                ...
                shares[t] = (nav * w) / buy_price
                hwm[t] = today[t]
            
            cash = nav - sum((shares[t] * apply_slippage(today[t], is_buy=True)) for t in alloc_w.keys())
    ...
    return pd.DataFrame(rows).set_index("date"), pd.DataFrame(alloc_rows)
...
def run_options_backtest(port_prices: pd.DataFrame, initial_capital: float) -> pd.DataFrame:
    tickers = port_prices.columns.tolist()
    ...
    for date, row in port_prices.iterrows():
        for t in tickers:
            ...
            should_roll = last_roll[t] is None or (date - last_roll[t]).days >= OPT_ROLL_DAYS
            ...
            if should_roll:
                ...
                opt_vals[t] = _spread_value(S, p["K1"], p["K2"], T_rem, RISK_FREE_RATE, max(sigma, 0.05)) * 100.0 * p["contracts"]
...
def compute_metrics(daily: pd.DataFrame) -> pd.DataFrame:
    labels = {"equity": "Dynamic Layering (50/30/20)", "options": "Options Path ($50 spreads)", "SPY": "SPY Benchmark", "QQQ": "QQQ Benchmark"}
    rows = []
    for col in [c for c in daily.columns if c.endswith("_nav")]:
        ...
        mdd_v = _max_dd(nav)
        rows.append({
            "Series": label, "Start ($)": f"{nav.iloc[0]:,.2f}", "End ($)": f"{nav.iloc[-1]:,.2f}",
            "Total Return": f"{nav.iloc[-1] / nav.iloc[0] - 1:+.1%}", "CAGR": f"{cagr_v:+.1%}",
            "Ann. Vol": f"{_ann_vol(ret):.1%}", "Sharpe": f"{_sharpe(ret):.2f}",
            "Sortino": f"{_sortino(ret):.2f}", "Max DD": f"{mdd_v:.1%}", "Calmar": f"{_calmar(cagr_v, mdd_v):.2f}",
        })
...
def compute_holdings(daily: pd.DataFrame, alloc_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for t, cfg in PORTFOLIO.items():
        ...
        rows.append({
            "Ticker": t, "Layer": cfg["layer"], "Description": cfg["desc"],
            "R1(50%) x": r1, "R2(30%) x": r2, "R3(20%) x": r3, "Stops": n_stops,
            "Days Held": int(held_mask.sum()), "Peak Pos ($)": f"{v.max():,.0f}", "P&L Contrib.": f"{holding_pnl / INITIAL_CAPITAL:+.1%}",
        })
    return pd.DataFrame(rows).set_index("Ticker")

def print_report(metrics, holdings, alloc_df, daily) -> None:
    SEP = "═" * 80
    print(f"\n{SEP}\n  DYNAMIC CAPITAL LAYERING PORTFOLIO – BACKTEST REPORT v3.1\n  Period: {START_DATE} → {END_DATE}\n  Capital: ${INITIAL_CAPITAL:,.2f} | Stop: {TRAIL_STOP_PCT:.0%} trailing\n{SEP}")
    print("\n▸ PERFORMANCE COMPARISON\n" + "─" * 80)
    print(metrics.T.to_string())
    print(f"\n{SEP}\n  Output files written: backtest_daily.csv, backtest_metrics.csv, backtest_holdings.csv, backtest_allocations.csv\n{SEP}")
...
def git_push(commit_msg: str) -> bool:
    try:
        ...
        branch = "HEAD"
    ok = True
    for cmd, label in [(["git", "add", "."], "git add ."), (["git", "commit", "-m", commit_msg], "git commit"), (["git", "push", "-u", "origin", branch], f"git push → {branch}")]:
...
def main() -> None:
    print("\n" + "━" * 80 + "\n  SIMULATION ENGINE v3.1 – Dynamic Capital Layering (Slippage Adjusted)\n" + "━" * 80)
    ...
    bm_prices = all_prices[[b for b inBENCHMARKS if b in all_prices.columns]].copy()
